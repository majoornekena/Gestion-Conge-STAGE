        ...
        <div class="container">
            <div class="row justify-content-center">
                <div class="col-12 text-center">
                    <h2 class="section-title">Ajout d'une facture</h2>
                </div>
                <!-- topic-item -->
                <div class="col-lg-12">
                    <form action="{{ url('/Ajout_FACTURE') }}" method="POST">
                        {{ csrf_field() }}
                        <div class="mb-3">
                            <label for="idpatient" class="form-label">Patient:</label>
                            <select name="idpatient" id="idpatient" class="form-control">
                                @foreach($patients as $patient)
                                <option value="{{ $patient->idpatient }}">{{ $patient->nom }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="mb-3">
                            <label for="datefacture" class="form-label">Date de la facture:</label>
                            <input type="date" name="datefacture" class="form-control" id="datefacture">
                        </div>
