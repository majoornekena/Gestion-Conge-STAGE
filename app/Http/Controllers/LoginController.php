<?php

namespace App\Http\Controllers;

use Illuminate\Http\Request;
use App\Admin;
use App\Employee;
use Illuminate\Support\Facades\Cache;
use Illuminate\Support\Facades\DB;
use Illuminate\Support\Facades\Hash;
use Illuminate\Support\Facades\Session;

use Illuminate\Support\Str;

class LoginController extends Controller
{
    public function showLoginForm()
    {
        return view('/login/Login');
    }

    public static function authenticate($mail, $mdp)
    {
        // Vérifie l'authentification pour l'admin
        $adminLogin = collect(DB::select('SELECT COUNT(*) AS count FROM admins WHERE users = ? AND mdp = ?', [$mail, $mdp]))->first();

        // Vérifie l'authentification pour l'employé
        $employeeLogin = collect(DB::select('SELECT COUNT(*) AS count FROM employes WHERE mail = ? AND mdp = ?', [$mail, $mdp]))->first();

        if ($adminLogin->count > 0) {
            // L'authentification correspond à un admin
            return 'admin';
        } elseif ($employeeLogin->count > 0) {
            // L'authentification correspond à un employé
            return 'employee';
        } else {
            // L'authentification a échoué
            return null;
        }
    }

    public function showSessionData(Request $request)
    {
        dd(session()->all());

    }
    
public function clearSessionToken(Request $request)
{
    // Supprime la session api_token
    $request->session()->flush();

}


public function login(Request $request)
{
    // Vérifie le profil d'authentification (admin, employé, etc.)
    $profil = self::authenticate($request->input('mail'), $request->input('mdp'));

    if ($profil === 'admin') {
        // Authentification réussie en tant qu'admin
        $adminId = Admin::where('users', $request->input('mail'))->value('idadmin');

        $adminToken = bin2hex(random_bytes(30));
        Admin::where('idadmin', $adminId)->update(['api_token' => $adminToken]);
        session(['api_token' => $adminToken]);

        return redirect('/admin/AjoutPatient');
    } elseif ($profil === 'employee') {
        // Authentification réussie en tant qu'employé
        $employeeToken = bin2hex(random_bytes(30));

        Employee::where('mail', $request->input('mail'))
                ->where('etatsup', 0) // Ajoutez la condition ici
                ->update(['api_token' => $employeeToken]);

        session(['api_token' => $employeeToken]);

        return redirect('/user/AjoutActe');
    } else {
        // L'authentification a échoué
        return redirect('/login')->with('erreur', 'Erreur, Email ou Mot de passe incorrecte. Veuillez réessayer.');
    }
}


    public function logout(Request $request)
    {
        // Détruit la session
        $request->session()->flush();

        // Redirige vers la page de connexion
        return redirect("/login");
    }


     public function showResetPasswordForm()
    {
        return view('login.resetpassword');
    }

    public function resetPassword(Request $request)
    {
        // Récupérer les données du formulaire
        $data = $request->all();
    
        // Trouver l'employé par son adresse e-mail
        $employee = Employee::where('mail', $data['mail'])->first();
    
        if ($employee) {
            // Mettre à jour le mot de passe de l'employé avec le nouveau mot de passe saisi dans le formulaire
            $employee->update(['mdp' => $data['mdp']]);
    
            return redirect('/login')->with('success', 'Mot de passe réinitialisé avec succès. Connectez-vous avec votre nouveau mot de passe.');
        } else {
            return redirect('/resetPassword')->with('erreur', 'Adresse e-mail incorrecte. Veuillez réessayer.');
        }
    }
    


    public function showRegisterForm()
    {
        return view('/login/Register');
    }
    public function register(Request $request)
    {
        if ($request->hasFile('imgprofile')) {
            // Obtenez le fichier téléchargé
            $image = $request->file('imgprofile');
    
            // Lisez le contenu du fichier en format base64
            $base64Image = base64_encode(file_get_contents($image->path()));
        } else {
            // Si aucun fichier n'a été téléchargé, utilisez une image par défaut en format base64
            $base64Image = '';
        }
        // Validez les données du formulaire
        $request->validate([
            // 'nom' => 'required|string|max:255',
            // 'prenom' => 'required|string|max:255',
            // 'imgprofile' => 'nullable|string|max:255',
            // 'sexe' => 'required|string|max:10',
            // 'mail' => 'required|string|email|max:255',
            // 'mdp' => 'required|string|min:6|max:30',
        ]);

        // Vérifiez si l'adresse e-mail est déjà utilisée dans la base de données
        $existingEmail = DB::table('employes')->where('mail', $request->input('mail'))->first();

        if ($existingEmail) {
            // Adresse e-mail déjà existante, redirigez avec un message d'erreur
            return redirect('/register')->with('erreur', 'Cette adresse e-mail est déjà utilisée. Veuillez en choisir une autre.');
           
        }

        // Créez un nouvel employé avec les données du formulaire
        $employee = new Employee([
            'nom' => $request->input('nom'),
            'prenom' => $request->input('prenom'),
            'datenaissance' => $request->input('datenaissance'), // Assurez-vous que votre formulaire ou requête inclut la date de naissance
            'imgprofile' => $base64Image,
            'sexe' => $request->input('sexe'),
            'residence' => $request->input('residence'), // Ajoutez le champ residence
            'phone' => $request->input('phone'), // Ajoutez le champ phone
            'mail' => $request->input('mail'),
            'mdp' => $request->input('mdp'),
            'api_token' => '0', // Valeur par défaut pour apitoken
            'etatsup' => 0, // Valeur par défaut pour etatsup
        ]);
        

        // Enregistrez le nouvel employé dans la base de données
        $employee->save();

        // Enregistrement réussi, redirigez avec un message de succès
        return redirect('/login')->with('success', 'Inscription réussie. Connectez-vous avec vos identifiants.');
    }
}